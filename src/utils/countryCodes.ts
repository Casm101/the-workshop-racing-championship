...
// Function to find a country given a name, alpha2 or alpha3 code
export const findCountry = (countryValue: string) => {
  let country;

  switch (countryValue.length) {
    case 2:
      country = countryCodes.find(country => country.alpha2 === countryValue.toUpperCase());
      break;
    case 3:
      country = countryCodes.find(country => country.alpha3 === countryValue.toUpperCase());
      break;
    default:
      country = countryCodes.find(country => country.name.toLowerCase() === countryValue.toLowerCase());
  };

  if (!country) {
    console.error('No country found for given value');
    return undefined;
  }
